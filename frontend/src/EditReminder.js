...
const EditReminder = () => {
  ...
  // Fetch existing reminder if editing
  useEffect(() => {
    if (reminderId) {
      const fetchReminder = async () => {
        try {
          const response = await axios.get(`http://localhost:5001/reminders/${reminderId}`);
          const reminderData = response.data;
          setFormData({
            ...reminderData,
            start_date: reminderData.start_date.split("T")[0], // Ensure start_date is in 'YYYY-MM-DD' format
          });
        } catch (err) {
  ...
  const validateForm = () => {
    ...
    if (!formData.times_of_day.trim()) newErrors.times_of_day = "Times of day is required"; // Add this line
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      try {
        // Update reminder
        await axios.put(`http://localhost:5001/reminders/${reminderId}`, formData);
        setSnackbar({
          open: true,
